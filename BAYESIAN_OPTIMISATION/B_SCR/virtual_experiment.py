...
def virtual_experiment_from_array(recalc_mape=None,
	...
	fixed_dic: k,v pairs for fixed parameters
	geom_dic: dictionary of k,v geometry information
	mat_dic: dictionary of k, v material properties information
	...
	if df.empty:
		...
		for rw, vals in prev_results.iterrows():
			...
			if recalc_mape == True:
				...
				optimizer.register(params=[vdic[k] for k in sorted(vdic.keys())], target=opt_target)
			else:
				opt_target = set_optimiser_target(target_header=target,
	...
	# ##SET THE INCREMENT NUMBER TO LAST DOE VALUE
		...
		for rw, vals in df.iterrows():
			...
			vdic = vals.to_dict()  # ##SORT KEYS BY CHANGING DF TO DIC
...
def virtual_experiment_from_new_point(phasei=None,
	...
	optimizer.register(params=[aba_dic[k] for k in sorted(optimizer.res[0]['params'].keys())], target=opt_target)
